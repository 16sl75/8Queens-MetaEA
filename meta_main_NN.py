import meta_evaluation_NN; import meta_initialization_NN; import random; import meta_mutation_NN
...
def meta_main():
    ...
    meta_popsize = 8; 
    meta_population = meta_initialization_NN.meta_permutation(meta_popsize)

    """EVALUATE each candidate"""
    meta_fitness_list = []
    for meta_individual in meta_population:
        print("\n\nindividual=", meta_individual)
        meta_fitness = meta_evaluation_NN.meta_evaluation(meta_individual) # you can change the parameters here
        print("meta_fitness[score]=", meta_fitness)
        print(f'optimizer_method:{meta_individual[0]}, learning_rate:{meta_individual[1]}, epochs:{meta_individual[2]}, batch_size:{meta_individual[3]}')
    ...
    while meta_gen < meta_gen_limit:
        ...
        meta_parents_index = meta_parent_selectionRLNN.meta_tournament(parent_fitness, meta_mating_pool_size, 8)
        ...
        while len(meta_offspring) < meta_mating_pool_size:
            ...
            if random.random() < meta_mut_rate:
                off1 = meta_mutation_NN.permutation_swap(off1)
            if random.random() < meta_mut_rate:
                off2 = meta_mutation_NN.permutation_swap(off2)
            # 4 EVALUATE new candidates;
            meta_offspring.append(off1)
            meta_offspring_fitness.append(meta_evaluation_NN.meta_evaluation(off1))
            meta_offspring.append(off2)
            meta_offspring_fitness.append(meta_evaluation_NN.meta_evaluation(off2))
            ip += 2
        # 5 SELECT individuals for the next generation;
    ...
    with open("result_NN.json", "w") as outfile:
